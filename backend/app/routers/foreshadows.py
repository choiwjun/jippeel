...
_SUGGEST_SYSTEM = (
    ...
    "떡밥이 없으면 빈 배열을 반환한다.\n"
    "후보는 단순한 수상한 단어가 아니라 이후 인물·관계·목표·권력·생존 상태를 바꿀 수 있는 정보여야 한다. "
    "본문에 드러난 표면 신호와 숨은 질문을 분리하고, 회수 조건과 회수 시의 변화를 추론하라. 어떤 조건에서 회수할지까지 구체화한다. "
    "새로운 복선 후보는 본문에 근거해야 하며, 본문 밖의 유행 요소나 고유명사를 추가하지 마라. "
    "복선은 향후 사건의 인과와 연결되어야 하며 단순 반전용 장식으로 만들지 마라."
)
...
async def suggest_foreshadows(pid: int, payload: ForeshadowSuggestRequest,
    ...
    messages = [
        ...
        {"role": "user", "content": (
            ...
            '다음 JSON 형식으로 출력하라:\n'
            '{"candidates": [{"title": "복선 제목", "content": "표면 신호·숨은 질문·향후 인과를 포함한 1~2문장", '
            '"keywords": ["본문 매칭용 키워드"]}]}')},
    ]

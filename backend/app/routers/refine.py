...
from app.services import humanize, manuscripts
...
def refine_chapter(payload: RefineRequest, db: Session = Depends(get_db)):
    ...
    input_content_md = chapter.content_md or ""
    base_revision = int(chapter.revision or 0)
    if base_revision != payload.expected_revision:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=manuscripts.RevisionConflict(current_revision=base_revision).detail(),
        )
    if not input_content_md.strip():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="본문이 비어 있어 윤문할 수 없습니다.")

    try:
        result = humanize.run_pipeline(input_content_md, payload.force_route)
...
@router.post("/refine/runs/{run_id}/accept", response_model=ChapterDetail)
def accept_refine_run(run_id: int, db: Session = Depends(get_db)):
    """FR-504 — 수락 → 대상 회차 본문 교체."""
    ...
    if run.base_revision is None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                            detail="기준 revision이 없는 기존 윤문 기록은 수락할 수 없습니다. 다시 윤문을 실행하세요.")

    try:
        chapter = manuscripts.replace_manuscript(
            db, run.chapter_id, run.result_text, run.base_revision, reason="refine"
        )
        run.accepted = True
        db.commit()
        db.refresh(chapter)

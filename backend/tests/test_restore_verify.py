...
def source_db(tmp_path: Path) -> Path:
    ...
    conn.execute("INSERT INTO alembic_version VALUES ('9d0e1f2a3747')")
    conn.execute(
...
def test_backup_and_verify_happy_path(source_db, tmp_path):
    ...
    assert manifest["alembic_head"] == "9d0e1f2a3747"
    assert manifest["format_version"] == 1
    names = {f["name"] for f in manifest["files"]}
    assert "database.sqlite3" in names

    report = verify_backup_dir(dest, expected_alembic_head="9d0e1f2a3747")

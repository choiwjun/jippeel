  ...
  await expect
    .poll(
      ...
      },
      {
        timeout: 90_000,
        message:
          "backend fixture pointer must reach holding status with complete seed IDs before tests read it",
      },
    )
    .not.toBe("");
  return readReport();
...
async function waitForKindCount(
  ...
  await expect
    .poll(
      () =>
        readPrompts(promptLog).filter((entry) => entry.kind === kind).length,
      {
        timeout: 20_000,
...
async function apiJson<T = unknown>(
  ...
  expect(
    response.status(),
    `${method} ${url} body=${await response.text().catch(() => "")}`,
  ).toBe(expected);
  return (await response.json()) as T;
}

async function creativeSnapshot(
  request: APIRequestContext,
  report: Report,
  ...
  const [characters, relationships, lore, foreshadows] = await Promise.all([
    ...
    apiJson(
      request,
      "GET",
      `/api/v1/characters/${report.character_ids[0]}/relations`,
    ),
    apiJson(request, "GET", `/api/v1/projects/${report.project_id}/lore`),
    apiJson(
      request,
      "GET",
      `/api/v1/projects/${report.project_id}/foreshadows`,
    ),
  ]);
  return {
...
async function prepareEditorPage(
  page: Page,
  report: Report,
  ...
  await expect
    .poll(
        ...
        }),
      {
        timeout: 20_000,
        message:
          "natural editor and AI store identity should bind from the real editor route",
      },
    )
    .toBe(
      `${report.project_id}:${report.chapter_id}:${report.project_id}:${report.chapter_id}`,
    );
}

async function setSelectionOnly(
  page: Page,
  report: Report,
  patch: Record<string, unknown> = {},
) {
  await page.evaluate(
    async ({ characterIds, patch }) => {
      const ai = await import("/src/stores/aiPanelStore.ts");
...
async function configureSharedUiControls(
  page: Page,
  report: Report,
  purpose: "serial" | "volume_end" | "series_finale",
) {
  ...
  const payoff = page.getByLabel(
    /이번 요청에서 회수\/공개 허용: 검의 진짜 주인/,
  );
  ...
  evidence.promptChecks = [
    ...(evidence.promptChecks as unknown[]),
    {
      phase: "ui-controls",
      purpose,
...
function querySqlite(report: Report, sql: string, params: unknown[] = []) {
  ...
  const out = execFileSync(
    ...
    ["-c", code, report.db_path, sql, JSON.stringify(params)],
    { encoding: "utf-8" },
  );
  return JSON.parse(out) as Array<Record<string, unknown>>;
}

async function expectNoProviderIncrease(
  report: Report,
  before: number,
  action: () => Promise<void>,
  note: string,
...
test.describe
  .serial("AI context consistency real Task4 integration", () => {
    test.beforeAll(async ({ request }) => {
      ...
      evidence.report = {
        ...
        ports: {
          frontend: report.frontend_port,
          backend: report.backend_port,
          provider: report.provider_port,
        },
        ...
        seed_ids: {
          ...
          future_resolution_foreshadow_id:
            report.future_resolution_foreshadow_id,
    ...
    test.afterAll(async ({ request }) => {
      ...
      evidence.finalPromptCounts = readPrompts(report.prompt_log).reduce<
    ...
    test("browser single generation + automatic review uses saved identity while body opt-out omits body sentinel", async ({
      page,
    }) => {
      const report = await waitForReadyReport();
      ...
      page.on("request", (req) => {
        if (
          req.method() === "POST" &&
          req.url().includes("/api/v1/ai/generate")
        ) {
          try {
            uiRequests.push({ url: req.url(), body: req.postDataJSON() });
          } catch {
            /* noop */
          }
        }
      ...
      await page
        ...
        .fill("AI_CONTEXT_SINGLE_REQUEST — 검의 주인을 결말에서 드러내라");
      ...
      await expect(page.getByText("AI_CONTEXT_REFINED")).toBeVisible({
        timeout: 30_000,
      });
      ...
      const requestBody = uiRequests.at(-1)!.body as {
      ...
      expect(requestBody.context.expected_revision).toBe(
      ...
      expect(requestBody.context.approved_foreshadow_ids).toEqual([
        report.foreshadow_id,
      ]);
      ...
      const singleReview = prompts.find(
        (entry) => entry.kind === "single_review",
      )!;
      for (const [phase, entry] of [
        ...
        (evidence.promptChecks as unknown[]).push({
          phase,
          kind: entry.kind,
          contains: ["finale", "style", "relationship", "approved-payoff"],
          omits: ["body-sentinel"],
        });
      }
      ...
      await expect(
        page.getByRole("button", { name: "↪ 끼워넣기" }),
      ).toBeDisabled();
      (evidence.uiRequests as unknown[]).push({
        phase: "single",
        requests: uiRequests,
      });
    });

    test("browser parallel generation and canon log every provider phase with context parity", async ({
      page,
      request,
    }) => {
      const report = await waitForReadyReport();
      ...
      page.on("request", (req) => {
        if (
          req.method() === "POST" &&
          (req.url().includes("/api/v1/ai/generate-parallel") ||
            req.url().includes("/api/v1/canon-check"))
        ) {
          try {
            uiRequests.push({ url: req.url(), body: req.postDataJSON() });
          } catch {
      ...
      await page
        .getByLabel("프롬프트 직접 입력")
        .fill("AI_CONTEXT_PARALLEL_REQUEST — 순서대로 두 장면으로 닫아라");
      await page.getByLabel("집필 모드").selectOption("parallel");
      await configureSharedUiControls(page, report, "series_finale");
      await page.getByRole("button", { name: "✨ 병렬 집필 시작" }).click();
      await expect(page.getByText("AI_CONTEXT_PARALLEL_REVIEW")).toBeVisible({
        timeout: 45_000,
      });
      const parallelRequest = uiRequests.find((entry) =>
        entry.url.includes("/api/v1/ai/generate-parallel"),
      )?.body as { context?: Record<string, unknown> } | undefined;
      ...
      expect(parallelRequest?.context?.expected_revision).toBe(
        report.chapter_revision,
      );
      expect(parallelRequest?.context?.episode_purpose).toBe("series_finale");
      expect(parallelRequest?.context?.approved_foreshadow_ids).toEqual([
        report.foreshadow_id,
      ]);
      expect(parallelRequest?.context?.include_relationships).toBe(true);
      expect(parallelRequest?.context?.character_ids).toEqual(
        report.character_ids,
      );
      ...
      const planner = promptsAfterParallel.find(
        (entry) => entry.kind === "planner",
      )!;
      const plannerJson = JSON.parse(planner.response_content ?? "{}") as {
        scenes: Array<Record<string, unknown>>;
      };
      expect(plannerJson.scenes.map((scene) => scene.order)).toEqual([1, 2]);
      expect(plannerJson.scenes.at(-1)?.ending_intent).toBeTruthy();
      const workers = promptsAfterParallel.filter(
        (entry) => entry.kind === "worker",
      );
      const parallelReview = promptsAfterParallel.find(
        (entry) => entry.kind === "parallel_review",
      )!;
      ...
      const workerResponses = workers
        .map((entry) => entry.response_content ?? "")
        .join("\n---\n");
      ...
      expect(
        reviewPromptText.indexOf("AI_CONTEXT_PARALLEL_SCENE_1"),
      ).toBeLessThan(reviewPromptText.indexOf("AI_CONTEXT_PARALLEL_SCENE_2"));

      await page.keyboard.press("Escape");
      await page.evaluate(
        async ({ projectId, chapterId, foreshadowId }) => {
          const ai = await import("/src/stores/aiPanelStore.ts");
        ...
        },
        {
          ...
          chapterId: report.chapter_id,
          foreshadowId: report.foreshadow_id,
        },
      );
      const savedChapterForCanon = await apiJson<{
        id: number;
        project_id: number;
        revision: number;
        content_md: string;
      }>(request, "GET", `/api/v1/chapters/${report.chapter_id}`);
      ...
      const expectedCanonHash = crypto
        .createHash("sha256")
        .update(savedChapterForCanon.content_md, "utf-8")
        .digest("hex");
      ...
      const canonProvenance = page.getByText(expectedCanonProvenance, {
        exact: true,
      });
      await expect(canonProvenance).toBeVisible({ timeout: 30_000 });
      const actualCanonProvenance = (
        await canonProvenance.textContent()
      )?.trim();
      expect(actualCanonProvenance).toBe(expectedCanonProvenance);
      await waitForKindCount(report.prompt_log, "canon", 1);

      const canonEntries = readPrompts(report.prompt_log).filter(
        (entry) => entry.kind === "canon",
      );
      ...
      const canonRuns = await apiJson<
        Array<{ chapter_id: number; context_json: Record<string, unknown> }>
      >(
        request,
        "GET",
        `/api/v1/canon-check/runs?chapter_id=${report.chapter_id}`,
      );
      ...
      expect(canonRuns[0].context_json.checked_input_revision).toBe(
        savedChapterForCanon.revision,
      );
      expect(canonRuns[0].context_json.checked_input_hash).toBe(
        expectedCanonHash,
      );
      const canonDbRows = querySqlite(
        report,
        "SELECT chapter_id, context_json FROM canon_runs WHERE chapter_id = ? ORDER BY id DESC LIMIT 1",
        [report.chapter_id],
      );
      const canonDbContext = JSON.parse(
        String(canonDbRows[0].context_json),
      ) as Record<string, unknown>;
      expect(canonDbRows[0].chapter_id).toBe(report.chapter_id);
      expect(canonDbContext.checked_input_revision).toBe(
        savedChapterForCanon.revision,
      );
      ...
      (evidence.uiRequests as unknown[]).push({
        phase: "parallel-canon",
        requests: uiRequests,
      });
    });

    test("purpose quality, invalid ownership/revision no-provider matrix, unbound default, and creative-state preservation", async ({
      request,
    }) => {
      const report = await waitForReadyReport();

      const serial = await apiJson<{
        metrics: Record<string, unknown>;
        suggestions: string[];
      }>(
        request,
        "GET",
        `/api/v1/chapters/${report.chapter_id}/quality?episode_purpose=serial`,
      );
      const volumeEnd = await apiJson<{
        metrics: Record<string, unknown>;
        suggestions: string[];
      }>(
        request,
        "GET",
        `/api/v1/chapters/${report.chapter_id}/quality?episode_purpose=volume_end`,
      );
      const finale = await apiJson<{
        metrics: Record<string, unknown>;
        suggestions: string[];
      }>(
        request,
        "GET",
        `/api/v1/chapters/${report.chapter_id}/quality?episode_purpose=series_finale`,
      );
      ...
      const history = await apiJson<
        Array<{ metrics_json: Record<string, unknown> }>
      >(
        request,
        "GET",
        `/api/v1/chapters/${report.chapter_id}/quality/history`,
      );
      expect(
        history.some((row) => row.metrics_json?.episode_purpose === "serial"),
      ).toBe(true);
      expect(
        history.some(
          (row) => row.metrics_json?.episode_purpose === "volume_end",
        ),
      ).toBe(true);
      expect(
        history.some(
          (row) => row.metrics_json?.episode_purpose === "series_finale",
        ),
      ).toBe(true);
      const qualityRows = querySqlite(
        report,
        "SELECT content_hash, metrics_json FROM quality_checks WHERE chapter_id = ? ORDER BY id",
        [report.chapter_id],
      );
      const trueHash = crypto
        .createHash("sha256")
        .update(report.sentinels.body, "utf-8")
        .digest("hex");
      expect(qualityRows.length).toBeGreaterThanOrEqual(2);
      for (const row of qualityRows) expect(row.content_hash).toBe(trueHash);
      expect(
        qualityRows.map(
          (row) => JSON.parse(String(row.metrics_json)).episode_purpose,
        ),
      ).toEqual(
        expect.arrayContaining(["serial", "volume_end", "series_finale"]),
      );
      evidence.quality = {
        serial: serial.metrics,
        volumeEnd: volumeEnd.metrics,
        finale: finale.metrics,
        qualityRows,
      };

      let before = countPrompts(report.prompt_log);
      await expectNoProviderIncrease(
        report,
        before,
        async () => {
          await apiJson(
            ...
            "/api/v1/ai/generate",
            {
              ...
              context: {
                project_id: report.project_id,
                chapter_id: report.foreign_chapter_id,
              },
            },
            422,
          );
          ...
          await apiJson(
            ...
            "/api/v1/ai/generate",
            {
              ...
              context: {
                project_id: report.project_id,
                chapter_id: report.chapter_id,
                character_ids: [report.missing_id],
              },
            },
            404,
          );
      ...
      await legacy.text();
      await expect
        .poll(() => countPrompts(report.prompt_log), { timeout: 20_000 })
        .toBeGreaterThan(legacyBefore);
      ...
      (evidence.promptChecks as unknown[]).push({
        phase: "unbound-default",
        kind: legacyPrompt.kind,
      ...
      expect(creativeAfter.relationships).toEqual(
        creativeBefore!.relationships,
      );

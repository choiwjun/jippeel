...
test.describe.serial('병렬 집필 AI 패널', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    for (const project of (await existing.json()) as Array<{ id: number; title: string }>) {
      if (project.title === PROJECT_TITLE) await request.delete(`/api/v1/projects/${project.id}`);
    }
    const projectResponse = await request.post('/api/v1/projects', { data: { title: PROJECT_TITLE } });
    expect(projectResponse.ok()).toBeTruthy();
    projectId = ((await projectResponse.json()) as { id: number }).id;
    const chapterResponse = await request.post(`/api/v1/projects/${projectId}/chapters`, {
  ...
  test('parallel mode sends settings, renders draft/review, and does not auto-write', async ({ page }) => {
    ...
    await page.route('**/api/v1/ai/generate-parallel', async (route) => {
      ...
      const body = sse([
        ['parallel_start', {
          model: 'medium-test', generation_reasoning_effort: 'medium',
          review_model: 'review-test', review_endpoint: 'review-endpoint',
          review_reasoning_effort: 'xhigh', worker_limit: 2,
          injected_lore: [], injected_foreshadows: [], injected_outline: null,
        }],
        ['planner_done', { scene_count: 2, scenes: [{ order: 1, title: '장면 1' }, { order: 2, title: '장면 2' }] }],
        ...
        ['review_start', { model: 'review-test', endpoint: 'review-endpoint', reasoning_effort: 'xhigh' }],
        ['review', { delta: '[감수]\n- 장면 전환을 확인했다.' }],
        ['done', '[DONE]'],
      ]);
    ...
    await page.getByLabel('프롬프트 직접 입력').fill('두 장면으로 짧게 작성하라.');
    await page.getByRole('button', { name: '✨ 병렬 집필 시작' }).click();

    await expect(page.getByRole('tab', { name: /감수 의견/ })).toBeVisible({ timeout: 15_000 });
    await expect(page.locator('pre')).toContainText('[감수]');
    await expect(page.locator('pre')).toContainText('장면 전환을 확인했다');
    await expect(page.locator('button', { hasText: '끼워넣기' })).toBeVisible();

    const chapter = await page.request.get(`/api/v1/chapters/${chapterId}`);
    expect(((await chapter.json()) as { content_md: string }).content_md).toBe('');
    expect(requests).toHaveLength(1);
    expect(requests[0].worker_limit).toBe(2);
    expect(requests[0].preset_id).toBeNull();
    expect((requests[0].review as { reasoning_effort: string }).reasoning_effort).toBe('xhigh');
  });
});

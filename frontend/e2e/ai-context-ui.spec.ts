  ...
  return page.evaluate(
    ([projectId, chapterId]) => {
      ...
      return {
        ...
        oldDraft: window.localStorage.getItem(
          `jippeel:manuscript-draft:v1:${projectId}:${chapterId}`,
        ),
      };
    },
...
async function expectSameRuntimeAndStaleEditor(
  page: Page,
  before: Awaited<ReturnType<typeof captureRuntimeEditorState>>,
) {
  await expect
    .poll(async () =>
      page.evaluate(() => {
...
async function selectTwoCharactersForAi(page: Page) {
  ...
  await page.evaluate(
    ([a, b]) => {
      ...
      store
        .getState()
        .setContext({ characterIds: [a, b], includeCharacters: true });
    },
    [CHAR_A, CHAR_B],
  );
}
...
async function setupFixture(page: Page): Promise<FixtureState> {
  const state: FixtureState = {
    chapters: new Map<number, Chapter>([
      ...
      [
        SECOND_CHAPTER_ID,
        chapter(SECOND_CHAPTER_ID, "2화", "두 번째 회차 원고", 1),
      ],
    ]),
    characters: new Map([
    ...
    lores: new Map([
      ...
      [
        PROJECT_2_LORE_ID,
        fixtureLore(PROJECT_2_LORE_ID, SECOND_PROJECT_ID, "사막 기록고"),
      ],
    ]),
    writes: [],
    heldWrites: [],
  ...
  await page.context().route("**/api/v1/**", async (route) => {
    ...
    if (
      method === "GET" &&
      path === `/projects/${SECOND_PROJECT_ID}/chapters`
    ) {
      return json(200, []);
    }
    ...
    if (method === "GET" && charactersMatch) {
      ...
      return json(
        200,
        [...state.characters.values()].filter(
    ...
    if (method === "GET" && loreListMatch) {
      ...
      return json(
        200,
        [...state.lores.values()].filter((l) => l.project_id === projectId),
      );
    }
    ...
    if (method === "GET" && path === `/chapters/${FIRST_CHAPTER_ID}/scenes`)
      return json(200, []);
    if (method === "GET" && path === `/chapters/${SECOND_CHAPTER_ID}/scenes`)
      return json(200, []);
    if (
      method === "GET" &&
      path === `/projects/${PROJECT_ID}/foreshadows/match`
    )
      return json(200, []);
    if (method === "GET" && path === `/projects/${PROJECT_ID}/foreshadows`) {
      expect(url.searchParams.get("status_filter")).toBe("설치");
      return json(200, [
        { id: FORESHADOW_ID, title: "검의 진짜 주인", status: "설치" },
      ]);
    }
    ...
    if (method === "POST" && path === "/ai/generate-parallel") {
      ...
      return route.fulfill({
        ...
        body: sse([
          ...
          ["message", { delta: "AI_CONTEXT_UI_PARALLEL" }],
          ["done", "[DONE]"],
        ]),
      });
    }
    if (method === "POST" && path === "/canon-check") {
      ...
      return json(200, {
        ...
        issues: [
          {
            quote: `CURRENT_CANON_QUOTE_${body.chapter_id}`,
...
async function fulfillNextSave(state: FixtureState, revision: number) {
  ...
  const next = {
    ...current,
    content_md: held.body.content_md,
    revision,
    updated_at: now(),
  };
...
async function openCharacterAiFromProject(
  page: Page,
  projectId: number,
  ...
  await expect(page).toHaveURL(
    new RegExp(`/projects/${projectId}/characters$`),
  );
  await page
    .getByRole("button", { name: `${characterName} 상세 열기` })
    .click();
  await expect(
    page.getByRole("heading", {
      name: new RegExp(`캐릭터 — ${characterName}`),
    }),
  ).toBeVisible();
...
async function openLoreAiFromProject(
  page: Page,
  projectId: number,
  loreTitle: string,
) {
...
test.describe
  .serial("AI-context Task3 fixture UI boundaries", () => {
    ...
    test("failed flush, close, and navigation cancel pending provider starts", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await page.goto(`/projects/${PROJECT_ID}/write`);
    ...
    test("result from another chapter blocks insert and replace but keeps copy enabled", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      ...
      await expect(
        page.getByRole("button", { name: /⤳ 선택 교체/ }),
      ).toBeDisabled();
    });

    test("active editor preview tab remains editor-bound while respecting body opt-out", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await page.goto(`/projects/${PROJECT_ID}/write`);
      ...
      await expect(
        page.getByText("첫 회차 서버 원고는 본문 opt-out 때 보내면 안 된다."),
      ).toBeVisible();
    ...
    test("canon and quality use shared purpose, payoff, relationship, flush revision, and hook applicability", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await openEditorAndPanel(page);
      ...
      await page.getByRole("button", { name: "문체 신호 참고 점수" }).click();
      await expect(page.getByText("해당 없음")).toBeVisible();
      expect(
        state.qualityRequests.some((q) =>
          q.includes("episode_purpose=series_finale"),
        ),
      ).toBeTruthy();
    ...
    test("canon relationship opt-in is project-level with zero selected generation characters", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await page.goto(`/projects/${PROJECT_ID}/write`);
      ...
      await page.getByRole("button", { name: /검사 실행/ }).click();
      await expect(page.getByText(/검사 기준/)).toContainText(
        `회차 #${FIRST_CHAPTER_ID}`,
      );
    ...
    test("canon relationship opt-in is project-level with one selected generation character", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await page.goto(`/projects/${PROJECT_ID}/write`);
      ...
      await page.evaluate((a) => {
        ...
        store
          .getState()
          .setContext({ characterIds: [a], includeCharacters: true });
      }, CHAR_A);
    ...
    test("generation relationship control stays disabled with zero or one selected character", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await openEditorAndPanel(page);
      ...
      expect(state.generateRequests).toHaveLength(1);
      expect(state.generateRequests[0].context.include_relationships).toBe(
        false,
      );
      expect(state.generateRequests[0].context.character_ids).toEqual([CHAR_A]);
    });

    test("quality dialog is purpose-only and does not expose relationship controls", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await page.goto(`/projects/${PROJECT_ID}/write`);
    ...
    test("character standalone AI after unresolved editor recovery does not flush or inherit stale chapter identity", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      ...
      await page.goto(`/projects/${PROJECT_ID}/write`);
      await expect(
        page.getByText("로컬 복구본과 서버 원고가 다릅니다."),
      ).toBeVisible();
      const beforeNav = await captureRuntimeEditorState(page);
      expect(beforeNav).toMatchObject({
        editorProjectId: PROJECT_ID,
        editorChapterId: FIRST_CHAPTER_ID,
      });
      ...
      expect(JSON.stringify(state.generateRequests[0].context)).not.toContain(
        "OLD_UNRESOLVED_RECOVERY_DRAFT",
      );
    });

    test("lore standalone AI after same-project editor visit does not flush or inherit stale chapter identity", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await page.goto(`/projects/${PROJECT_ID}/write`);
    ...
    test("cross-project lore standalone AI uses explicit page project and not stale editor project or chapter", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      await page.goto(`/projects/${PROJECT_ID}/write`);
    ...
    test("delayed canon result from chapter A is ignored after navigating to chapter B", async ({
      page,
    }) => {
      const state = await setupFixture(page);
    ...
    test("closing A, visiting B, and returning A does not revive a cancelled canon result", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      state.holdCanon = true;
    ...
    test("older delayed canon success or failure cannot override a newer completed canon result", async ({
      page,
    }) => {
      const state = await setupFixture(page);
      state.holdCanon = true;
      ...
      await page.getByRole("button", { name: /검사 실행/ }).click();
      await expect(
        page.getByText(`CURRENT_CANON_QUOTE_${SECOND_CHAPTER_ID}`),
      ).toBeVisible();
      await expect(page.getByText(/검사 기준/)).toContainText("rev 1");
      expect(state.canonRequests[1]).toMatchObject({
      ...
      await expect(
        page.getByText("OLDER_SUCCESS_AFTER_NEWER"),
      ).not.toBeVisible();
      ...
      await expect(
        page.getByText(/OLDER_FAILURE_AFTER_NEWER/),
      ).not.toBeVisible();
    });
  });

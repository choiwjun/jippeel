...
export interface StreamHandlers {
  ...
  onPlannerDone?: (info: {
    sceneCount: number;
    scenes: Array<{ order: number; title: string }>;
  }) => void;
      ...
      if (eventName === "start") {
        try {
          ...
          handlers.onStart?.({
            ...
            injectedOutline:
              typeof parsed.injected_outline === "object" &&
              parsed.injected_outline !== null
                ? parsed.injected_outline
                : null,
            reviewEnabled: parsed.review_enabled === true,
            contextMetadata:
              typeof parsed.context_metadata === "object" &&
              parsed.context_metadata !== null
                ? parsed.context_metadata
                : undefined,
          });
        } catch {
          /* noop */
        ...
        try {
          ...
          handlers.onParallelStart?.({
            ...
            reviewProvider: parsed.review_provider ?? "",
...
export function streamParallelGenerate(
  body: unknown,
  handlers: StreamHandlers,
): () => void {

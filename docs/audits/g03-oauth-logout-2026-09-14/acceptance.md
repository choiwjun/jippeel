...
- 서버 측 토큰 폐기(revocation)는 이 도구가 노출하지 않는다. 특정 ChatGPT 계정의
  브라우저 세션을 임의로 로그아웃시키지 않는 것을 안전한 운영 방향으로 결정한다.
  서버 세션 폐기·완전 재로그인은 bridge 소유 G03의 차단 조건이 아니라 계정 소유자가
  원할 때 수행하는 비차단 후속 절차로 분리한다.
...
복구(복원 → 실 호출 성공)가 실증됐다. bridge 범위의 G03 수용은 **완료**다. 서버 측
세션 폐기와 완전 재로그인은 도구가 제공하지 않는 계정 소유자 선택 절차이며, 이번
수용을 막지 않는 비차단 항목으로 분리했다.

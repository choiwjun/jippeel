...
- 수용일: 2026-09-14
- 상태: **자동화 가능 영역 통과** — NVDA 실제 음성 발화·포커스 순환 실증(아래), 지정 장치 최종 사인오프만 잔여
...
긴 무중단 Tab 사이클은 확보 못 함 — 발화·순환 자체는 실증됐으나
지정 장치에서의 최종 대화형 사인오프는 별도로 남긴다.

## 잔여 — 수동 영역

- 지정 장치 최종 사인오프 — NVDA 실측은 위 증거로 실증됐으나, 사용자 지정
  Windows 장치에서의 대화형 최종 확인 절차가 별도 게이트로 남음
